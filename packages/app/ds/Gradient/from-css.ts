...
const parseGradient = (
  { args, method, prefix }: { args: Array<string>; method: string; prefix?: string },
  bounds: Bounds
) => {
  ...
  } else if (method === 'radial-gradient' || (method === 'gradient' && args[0] === 'radial')) {
...
const parseLinearGradient = (args: Array<string>, bounds: Bounds, hasPrefix: boolean) => {
 ...
 * ```
 */
export function gradientPropsFromCSS(str: string) {
  if (str.endsWith(';')) {
    str = str.slice(0, -1)
  }
